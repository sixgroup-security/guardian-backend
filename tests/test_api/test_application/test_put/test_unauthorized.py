...
from schema.application import ApplicationRead
...
def test_application(
        ...
        application_update_permission_fixture,
        get_application_fixture,
        put_application_fixture,
    ...
    if not application_update_permission_fixture(user_name=user_name):
        # First, we create an application
        application = get_application_fixture()
        response = post_application_fixture(user_name=GuardianRoleEnum.admin.name, data=application)
        assert response.status_code == status.HTTP_200_OK
        result = ApplicationRead(**response.json())
        # Then, we update the application
        application["id"] = str(result.id)
        response = put_application_fixture(user_name=user_name, data=application)

...
def test_post_provider(
        provider_create_permission_fixture,
        get_provider_fixture,
        post_provider_fixture,
        user_name
):
    """
    Tests that an unauthorized user can create a new provider.
    """
    # Check if the user has the required permission
    if not provider_create_permission_fixture(user_name=user_name):
        provider = get_provider_fixture()
        response = post_provider_fixture(user_name=user_name, data=provider)

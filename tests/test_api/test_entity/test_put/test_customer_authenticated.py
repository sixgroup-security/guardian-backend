...
from schema.entity import CustomerRead, CustomerCreate
...
def test_post_customer(
        customer_update_permission_fixture,
    ...
    """
    Tests that an authenticated user can update an existing customer.
    """
    # Check if the user has the required permission
    if customer_update_permission_fixture(user_name=user_name):
        ...
        response = put_customer_fixture(user_name=user_name, data=customer)
        assert response.status_code == status.HTTP_200_OK
        result = CustomerRead(**response.json())
        original = CustomerCreate(**customer)

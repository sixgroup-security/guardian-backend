...
def test_project(
        project_create_permission_fixture,
        get_project_fixture,
        post_project_fixture,
        user_name
):
    """
    Tests that an unauthorized user can create a new project.
    """
    # Check if the user has the required permission
    if not project_create_permission_fixture(user_name=user_name):
        project = get_project_fixture()
        response = post_project_fixture(user_name=user_name, data=project)
